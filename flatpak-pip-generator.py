...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help=("Do not disable build isolation. Mostly useful on pip that does't support the feature."),
)
parser.add_argument(
    "--ignore-installed",
...
def get_poetry_deps(pyproject_data: dict[str, Any]) -> list[str]:
    ...
    return sorted(format_dependency_version(dep, val) for dep, val in poetry_deps.items() if dep != "python")
...
def handle_req_env_markers(requirements_text: str) -> str:
    def handle_sys_platform(marker: str) -> bool:
        ...
        for match in re.finditer(pattern, marker, re.IGNORECASE):
            ...
            if (op == "==" and not platform.startswith("linux")) or (op == "!=" and platform.startswith("linux")):
                return False
        return True
    ...
    def handle_implementation_name(marker: str) -> bool:
        ...
        pattern_platform_impl = r'platform_python_implementation\s*(==|!=)\s*["\']([^"\']+)["\']'
        ...
        if current_platform_impl != "cpython":
            print(
                f"WARNING: platform.python_implementation() '{current_platform_impl}' does not match fdsdk runtime",
                file=sys.stderr,
            )

        for match in re.finditer(pattern_impl_name, marker, re.IGNORECASE):
            op, expected = match.group(1), match.group(2).lower()
            if (op == "==" and expected != current_impl_name) or (op == "!=" and expected == current_impl_name):
                return False

        for match in re.finditer(pattern_platform_impl, marker, re.IGNORECASE):
            op, expected = match.group(1), match.group(2).lower()
            if (op == "==" and expected != current_platform_impl) or (op == "!=" and expected == current_platform_impl):
                return False
...
if opts.requirements_file:
    ...
    if opts.ignore_pkg:
        ...
        dependencies = [dep for dep in dependencies if dep.split(" ")[0] not in opts.ignore_pkg]
...
with tempfile.TemporaryDirectory(prefix=tempdir_prefix) as tempdir:
    ...
    vcs_packages: dict[str, dict[str, str | None]] = {
        str(x.name): {"vcs": x.vcs, "revision": x.revision, "uri": x.uri} for x in packages if x.vcs and x.name
    }
...
for package in packages:
    ...
    with tempfile.TemporaryDirectory(
        ...
        try:
            ...
            for filename in sorted(os.listdir(tempdir)):
                ...
                if dep_name.casefold() in system_packages and dep_name.casefold() not in opts.ignore_installed:
                    continue
    ...
    for dependency in dependencies:
        ...
        elif casefolded.replace("_", "-") in sources and sources[casefolded.replace("_", "-")].get("pypi") is True:
            source = sources[casefolded.replace("_", "-")]
        elif dependency.replace("_", "-") in sources and sources[dependency.replace("_", "-")].get("pypi") is False:
